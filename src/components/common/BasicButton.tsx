  ...
  (
    ...
    const renderClassBySize = () => {
      switch (size) {
        case 'sm':
          // height: 32px
          return `h-[32px] ${variant !== 'text' && 'px-[14px]'} text-14-18 ${isRadius100 ? 'rounded-[100px]' : 'rounded-[8px]'}`
        case 'md':
          // height: 40px
          return `h-[40px] ${variant !== 'text' && 'px-[16px]'} text-14-18 ${isRadius100 ? 'rounded-[100px]' : 'rounded-[8px]'}`
        case 'lg':
          // height: 44px
          return `h-[44px] ${variant !== 'text' && 'px-[16px]'} text-16-20 ${isRadius100 ? 'rounded-[100px]' : 'rounded-[12px]'}`
        case 'xl':
          // height: 48px
          return `h-[48px] ${variant !== 'text' && 'px-[24px]'} text-16-20 ${isRadius100 ? 'rounded-[100px]' : 'rounded-[12px]'}`
        case 'xxl':
          // height: 64px
          return `h-[64px] ${variant !== 'text' && 'px-[28px]'} text-20-224 ${isRadius100 ? 'rounded-[100px]' : 'rounded-[16px]'}`
        default:
          return ''
      }
    }
    ...
    const colorLoading =
      (variant === 'outlined' && color === 'black') || (variant === 'contained' && color === 'white')
        ? '#000000'
        : '#FFFFFF'
    ...
    const renderButton = () => (
      ...
      >
        {startIcon}
        <span className={clsx({ 'ml-[8px]': !!startIcon, 'mr-[8px]': !!endIcon }, 'flex items-center font-inter-500')}>
          {props.children}
        </span>
        {endIcon}
        {loading && (
